...
The example uses a small **Workshop Supply Planner** to introduce specification-driven development with an AI coding assistant. Students begin with a compiling Java project whose `Workshop` class contains incomplete methods. The goal is to implement those methods incrementally until the program output matches the written specification.

> This repository currently contains the Day 1 documentation. The Java starter files described in [`docs/STARTER-CODE-PLAN.md`](docs/STARTER-CODE-PLAN.md) will be added separately.
...
Once the Java starter files are present:

1. Clone the repository and open it in IntelliJ IDEA.
2. Read [`docs/SPEC.md`](docs/SPEC.md) before prompting an AI assistant.
3. Run `Main` and observe that the project compiles but produces incomplete output.
4. Locate the TODO comments in `Workshop.java`.
...
2. `totalSupplyCount`
3. `scaleToAttendees`
4. private amount-formatting helper
5. `toString`
6. `toPrettyString`
